...
//			double weight = (double)(grayl < grayb ? Weighting : WeightingXOR) * weightNorm;
            ...
            BYTE rr = (intWeight * abs(rb - rl) + std::min((int)rb, (int)rl)) >> 8;
            BYTE gr = (intWeight * abs(gb - gl) + std::min((int)gb, (int)gl)) >> 8;
            BYTE br = (intWeight * abs(bb - bl) + std::min((int)bb, (int)bl)) >> 8;
